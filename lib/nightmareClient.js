...
function Nightmare(...args) {
    Nightmare.super_.apply(this, args);
...
exports.Nightmare = args => {
     ...
     */
    Nightmare.prototype.sendMessageToParent = msg => {
        socket.emit('msg', msg);
    };
     ...
     * @param   String      expectedTerm      A filter on the data type to be returned.
     * @param   Function    then            The next step to be preformed. (optional)
     * @param   Function    onTimeout       A callback function to call on timeout. (optional)
     * @param   Number      timeout         The max amount of time to wait, in milliseconds (optional)
     * @return  Nightmare
     */
    Nightmare.prototype.waitForMessageResponse = function waitForEmit(msgForParent, expectedTerm, then, timeout, onTimeout) {
        "use strict";
        this.checkStarted();
        ...
        return this.then(function _step() {
            ...
            this.on('msg.received', function(data){
                if(data[expectedTerm] !== undefined) {
                    condition = true;
