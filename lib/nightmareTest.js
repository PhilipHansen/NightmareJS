...
var casper = require(phantom.args[3].slice(13)+'nightmareClient.js').Nightmare({
    exitOnError: false
});
...
// includes handling
this.loadIncludes.forEach(include => {
    "use strict";
    var container;
    if (casper.cli.has(include)) {
        container = casper.cli.get(include).split(',').map(file => checkIncludeFile(file)).filter(file => utils.isString(file));
